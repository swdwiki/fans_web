  ...
  <div>
    ...
    <div class="web-list bg-white mb-5">
      <ul class="url-list w-full flex flex-col justify-start py-3">
        <li v-for="(url, index) in indexUrlList" :key="index" class="pl-3 url block hover:bg-gray-200">
          ...
          >
            <div v-if="url.icon">
              <icon-font :type="url.icon" :size="40" />
            </div>
            <div v-else>
              <img class="url-cover" :src="url.cover" :alt="url.name" />
            </div>
            <span class="ml-3 text-base">{{ url.name }}</span>
          </a>
          <a-popover v-else>
    ...
    <div class="web-info">
      ...
      <div class="info">
        ...
        <!-- <span>©{{ new Date().getFullYear() }} 轩辕天书 All Rights Reserved</span> -->
      </div>
    </div>
  </div>
</template>
...
const indexUrlList = ref<Array<any>>([
  {
    name: '轩辕剑官方微博',
    icon: '',
    cover: 'http://files.swdwiki.com/tos-cn-i-8vrofckkw9/avatar/swdmain.jpeg',
    showImg: false,
  ...
  },
  {
    name: '轩辕剑官方微信公众号',
    icon: '',
    cover: 'http://files.swdwiki.com/tos-cn-i-8vrofckkw9/avatar/wxicon.png',
    showImg: true,
    url: 'https://store.steampowered.com/app/1638230/_/?curator_clanid=4777282',
    imgUrl: 'http://files.swdwiki.com/tos-cn-i-8vrofckkw9/qrcode/qrcode.bmp',
  },
  // {
  //   name: '轩辕天书官方微博',
  //   icon: 'swd-weibo',
