<template>
  ...
  >
    ...
    <div class="right flex flex-col items-start justify-between w-[100%]" :class="[post.cover ? 'ml-5 ' : '']">
      <router-link v-if="props.post.state === 3" :to="{ name: 'PostDetail', params: { postId: post.postId } }">
        <div class="font-bold text-black text-base block leading-7 flex flex-row justify-start items-center">
          <a-tag v-if="props.post.history" class="mr-2" :color="STATUS_LIST[props.post.state].color" size="small">{{
            STATUS_LIST[props.post.state].label
          }}</a-tag>
            ...
            <a-trigger position="bottom" trigger="click" auto-fit-position :unmount-on-close="false">
              ...
              <template #content>
                <div class="demo-basic flex flex-col justify-center items-center text-sm">
                  <span
                    v-if="props.post.history && props.post.state === 4"
                  ...
                  <span
                    ...
                    >文章编辑</span
                  >
                  <a-popconfirm content="确定要上架/发布该文章么？" @ok="upPost(props.post.postId)">
                    <span
                      v-if="props.post.history && props.post.state === 2"
                      class="block leading-10 hover:bg-gray-100 w-full text-center"
                      >文章上架/发布</span
                    >
                  </a-popconfirm>
                  <a-popconfirm content="确定要下架该文章么？" @ok="downPost(props.post.postId)">
                    <span
                      v-if="props.post.history && props.post.state === 3"
                      class="block leading-10 hover:bg-gray-100 w-full text-center"
                      >文章下架</span
                    >
                  </a-popconfirm>
                  <a-popconfirm content="确定要删除该文章么？" @ok="deletePost(props.post.postId)">
                    <span class="block leading-10 hover:bg-gray-100 w-full text-center">文章删除</span>
                  </a-popconfirm>
                </div>
              </template>
            </a-trigger>
...
interface reportFormDto {
  reasonId: number | undefined;
  content: string;
  imgFilesUrl: Array<string>;
}
