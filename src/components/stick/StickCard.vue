<template>
  <div class="stick-card rounded-md bg-white mb-[10px] block">
          ...
          <div v-if="stick.likeCount && stick.likeCount !== 0 && stick.likes && stick.likes.length !== 0">
            <a-avatar-group>
              <a-avatar v-for="like in stick.likes" :key="like.recordId" :size="24">
                <img :draggable="false" :src="like.likeUser.avatar" />
              </a-avatar>
            </a-avatar-group>
    ...
    >
      <a-trigger trigger="click" show-arrow :popup-translate="[0, 10]" position="br">
        <div
        ...
        <template #content>
          <div
            class="bg-white w-[200px] h-[40px] leading-[40px] text-center border-gray-200 border flex flex-row items-center justify-center"
            @click="copyLink('sss')"
          >
      ...
      <template v-if="!showCommentList">
        <div
          class="w-1/3 block text-center flex flex-row justify-center items-center text-stroke-emerald-50 cursor-pointer"
