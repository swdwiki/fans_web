    ...
    <div class="submit-stick bg-white w-full pb-5">
      <div class="w-full mx-auto px-5 group py-[20px]">
        ...
        >
          <div
            ref="editorRef"
            class="rich-input transition duration-150 ease-out focus:ease-in p-2 focus:duration-300 focus:bg-white focus:h-[120px] transtion-all ease-in duration-100 text-sm leading-6 h-[100px] outline-none rounded-tl-md rounded-tr-md overflow-scroll w-full"
            contenteditable
            :spellcheck="false"
          ...
          <div class="flex flex-row justify-between items-end">
            <div>
              <div class="mb-[10px]">
                ...
                >
                  <span class="bg-gray-50 py-[5px] px-[10px] text-xs rounded-xl text-blue-500 ml-[5px]">
                    <span class="mr-[3px]" @click="showClubSelect = true">{{
                      selectClubThemeText || '请选择圈子'
                    }}</span>
                    <icon-right :size="12" />
                  </span>
                  <template #content>
                    <div class="demo-basic bg-white rounded-sm border border-gray-200">
                      <div class="list">
                        <div class="topic-list flex flex-row h-[380px] w-[400px] rounded-sm overflow-scroll">
                          <div class="left flex flex-col py-[10px] bg-gray-100 border-r border-r-gray-200">
                            <span
                              v-for="(club, index) in clubs"
                              :key="index"
                              class="h-[48px] w-[100px] leading-[48px] text-center"
                          ...
                          <div
                            class="w-[300px] h-full right flex flex-col justify-start items-start py-[10px] px-[10px] overflow-scroll"
                          >
                            <div
                              v-for="theme in clubs[clubParentIndex].themes"
                              :key="theme.themeId"
                              class="w-full min-h-[60px] h-[60px] flex flex-row items-center justify-start"
                            ...
                            >
                              ...
                              <div class="ml-[10px] h-full topic-info flex flex-col justify-center items-start">
                                <span class="font-bold block text-base">{{ theme.name }}</span>
                                <span class="block text-gray-400 text-xs"
                                  >{{
                                    theme.stickCount < 1000
                                      ? theme.stickCount
                                      : (theme.stickCount / 1000).toFixed(1) + 'k'
                                  }}尺牍</span
                                >
                              </div>
                            </div>
                        ...
                        <div
                          class="no-select h-[70px] border-t border-t-gray-200 flex flex-row justify-end items-center px-[20px]"
                        >
                          <a-button type="primary" @click="noSelectClubThemes">不选择圈子</a-button>
                        </div>
                      </div>
                    </div>
                  </template>
...
const popupVisibleChange = (type: boolean) => {
  // showClubSelect.value = type;
