<template>
  <div class="message-container">
    <div class="top-nav-bar">
      <div class="nav-list">
        <router-link
          v-for="(nav, index) in messageNavList"
          :key="index"
          :to="nav.params ? { name: nav.router, query: { type: nav.params as string } } : { name: nav.router }"
          :class="[route.name === nav.router ? 'nav-selected' : '', 'nav-item']"
          >{{ nav.name }}
          <a-tag
            v-if="injectHandle.returnCommentCount() && injectHandle.returnCommentCount()[nav.key]"
            color="#f53f3f"
            class="ml-[5px]"
            size="small"
            >{{ injectHandle.returnCommentCount()[nav.key] }}</a-tag
          >
        </router-link>
      </div>
    </div>
...
const messageNavList = ref([
  {
    name: '收到的评论',
    router: 'CommentMessage',
    key: 'comment',
    params: 'post',
  },
  {
