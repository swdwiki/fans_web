      ...
      <div class="flex flex-col justify-center items-center">
        ...
        <div class="handle flex flex-row justify-around items-center">
          <a-button
            ...
            type="primary"
            class="mx-[10px]"
            @click="gotoUrl('workDetail', { id: submitWorkId })"
            >查看作品详情</a-button
          >
        ...
        <div class="form-wrap">
          ...
          <div class="work-options">
            <div class="mb-[60px] p-[20px]">
              ...
              <a-form-item label="封面图" field="cover" :label-col-style="labelStyle">
                ...
                >
                  <template #upload-button>
                    <div v-if="submitWorkForm.cover" class="upload-cover">
              ...
              <template v-if="showR18OptionStatus">
                <a-form-item label="含18+内容" field="isR18" :label-col-style="labelStyle">
...
function customParseVideoSrc(src: string): string {
  ...
  if (src.includes('.bilibili.com')) {
    ...
    return `<iframe src="//player.bilibili.com/player.html?bvid=${arr[2]}&autoplay=0" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true" width="800" height="400" style="width: 100%; height: 500px; max-width: 100%；align:center; padding:20px 0;"> </iframe>`;
  }
  return src;
}
...
const editorConfig: any = {
  ...
  MENU_CONF: {
    ...
    uploadImage: {
      ...
      meta: { prefix: 'image' },
      maxNumberOfFiles: 9,
      base64LimitSize: 1 * 1024, // 小于 1KB 的文件转为 base64 格式
      headers: {
        ...headers.value,
        // 'Content-Type': 'multipart/form-data',
      },
      onBeforeUpload(file: File) {
        imgIndex.value.push(1);
        return file;
      },
      ...
      async customUpload(file: any, insertFn: InsertFnType) {
        ...
        formData.append('file', file); // 字段名需和后端一致
        try {
          const res = await axios(fileUploadUrl, {
            method: 'POST',
            headers: {
              ...headers.value,
            },
          ...
          if (res.data.data.url) {
            insertFn(res.data.data.url, submitWorkForm.value.title || userInfo.nickname || '', res.data.data.url);
          }
...
const selectWorkCate = (value: any) => {
  ...
  console.log(findSonCate, 'findSonCate');
  if (findSonCate) {
    if (findSonCate.needCover) {
      ...
      //   delete formRules.value.cover;
      formRules.value.cover = [];
    }
    console.log(findSonCate.r18);
      ...
      workApi
        ...
        .then((res) => {
          ...
          if (res.code === 0) {
            ...
            submitWorkId.value = res.data.workId;
          } else {
            Message.error(res.message);
