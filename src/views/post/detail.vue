<template>
  <div class="post_detail_wrap w-[1200px] mx-auto flex flex-row justify-center items-start mb-10">
    ...
    <div class="post_info w-[300px] flex flex-col sticky top-[80px] ml-[10px]">
      <div class="author_card bg-white mb-5 p-5">
        ...
        <div class="amount_list flex flex-row flex-nowrap justify-around items-center px-5 my-2">
          <div class="flex flex-col justify-center items-center">
            ...
            <span class="text-sm text-gray-400 my-1 font-bold">文章</span>
          </div>
          <div class="flex flex-col justify-center items-center">
        ...
        <div class="card_handle">
          <a-button v-if="!followStatus" class="w-full" size="large" type="primary" @click="follow">关注</a-button>
          <a-popconfirm v-else content="确定取消关注么？" @ok="follow">
            <a-button
              class="w-full"
              size="large"
      ...
      <div v-if="postDetail.columns && postDetail.columns.length !== 0" class="column_card bg-white px-5 py-3 mb-5">
        ...
        <a-divider :margin="10" />
        <router-link :to="`/column/${postDetail.columns[0].columnId}`" class="flex flex-row justify-start items-center">
          <img :src="postDetail.columns[0].cover" class="w-[80px] h-[60px] object-cover" />
          <div class="column_info flex flex-col ml-3 justify-center items-start">
            ...
            <span class="text-xs text-gray-400"><span class="mr-1">10</span>篇文章</span>
          </div>
        </router-link>
      </div>
      <div class="">
        <a-collapse :default-active-key="[1]" :bordered="false" expand-icon-position="right">
          ...
          <a-collapse-item :key="1" :style="customStyle">
            ...
            <div class="">
              ...
              <!-- <template v-if="postDetail.history[0].editorType === 'rich'">
...
const follow = () => {
  if (isLoginStatus) {
    defaultApi
      .follow({ followId: postDetail.value.author.userId })
      .then((res) => {
        if (res.code === 0) {
          if (res.data) {
            Message.success('已关注');
