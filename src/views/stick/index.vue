  ...
  <div class="stick-container relative mx-auto relatice">
    <div class="stick_clubs bg-white w-[180px] py-[10px] sticky top-[80px] left-0 h-[640px] overflow-scroll">
      <div class="w-[165px] mx-auto">
        <!-- <a-menu :style="{ height: '100%' }" :default-open-keys="['0']" :default-selected-keys="['0_1']" auto-open>
          <a-menu-item key="0">最新尺牍</a-menu-item>
          <a-sub-menu v-for="club in clubList" :key="String(club.clubId)">
            <template #title>{{ club.name }}</template>
            <a-menu-item v-for="theme in club.themes" :key="String(club.clubId) + String(theme.themeId)">
    ...
    <div ref="stickListRef" class="stick-list top-[80px] left-[200px]">
      ...
      />
      <div ref="stickListRef" v-loading="loading" class="list w-[720px] my-[20px] relative w-full">
    ...
    <div class="side-bar w-[260px] top-[80px]">
      <div class="user-box bg-white px-[10px] py-[20px]">
        ...
        <div class="count flex flex-row flex-nowrap justify-center items-center">
          <div class="count-card flex flex-col items-center w-1/3">
      ...
      <div v-if="recommendSticks && recommendSticks.length !== 0" class="bg-white mt-[20px]">
        ...
        <div class="flex flex-col px-[10px]">
          <div v-for="stick in recommendSticks" :key="stick.stickId" class="stick-card-recommend my-[10px]">
...
const publicStickCommentPagination = reactive({
  ...basePagination,
});
    ...
    .then(async (res: any) => {
      if (res.code === 0) {
        ...
        try {
          ...
          stickCardRef.value[stickIndex].comments = {
            list: data.list,
            total: data.total,
          };
        } catch (err) {
