<template>
  <div v-loading="loading" class="editor">
    <div class="header flex flex-row justify-between items-center">
      <a-input v-model="updatePostForm.title" class="title-input" placeholder="请输入帖子标题" size="large"></a-input>
      <div class="pr-5">
        <!-- <a-popconfirm
        ...
        >
          <a-button class="mx-2" type="primary" size="small">更新</a-button>
          <template #content>
            <div class="demo-arrow w-100">
              <div class="header h-12 border-solid border-b-gray-200 border-b w-full p-0">
                <span class="leading-[3rem] block font-bold text-base text-center">帖子更新</span>
              </div>
              <div class="w-11/12 mx-auto my-5">
                <a-form ref="postFormRef" :model="updatePostForm" auto-label-width>
                  <a-form-item
                    field="subjectId"
                    label="分类"
                    :rules="[
                  ...
                  >
                    <a-radio-group v-model="updatePostForm.subjectId" type="button" @change="changeSubject">
                  ...
                  >
                    <a-select v-model="updatePostForm.plateId" placeholder="请选择板块" :allow-create="false">
                      <a-option
                  ...
                  <a-form-item field="tagList" label="标签">
                    <a-select
                      v-model="updatePostForm.tagList"
                    ...
                    >
                      <template #header>
                        <span class="text-sm leading-8 h-8 text-gray-600 ml-5"
                          >最多只能选择或输入3个标签，已选择或输入{{
                            updatePostForm.tagList && updatePostForm.tagList.length
                          }}个标签</span
                        >
                      </template>
                      <a-option
                  ...
                  <a-form-item
                    field="columns"
                    label="收录专栏"
                    :rules="[{ type: 'array', maxLength: 1, message: '最多选择一个专栏' }]"
                    :validate-trigger="['change', 'input']"
                  >
                    <a-select
                      v-model="updatePostForm.columnId"
                      placeholder="请选择专栏"
                      :multiple="false"
              ...
              <div class="footer border-t border-t-gray-100 flex flex-row flex-nowrap justify-end items-center py-5">
                ...
                <a-button class="mr-5 ml-2" type="primary" :loading="loading" @click="submitPost">确定并更新</a-button>
              </div>
            </div>
          </template>
        </a-trigger>
    ...
    <div class="editor">
        ...
        <div class="md-help">
          ...
          <ul>
            ...
            <li>
              ...
              <div class="help-title">代码</div>
              <div class="help-content"><code>`代码`</code></div>
            </li>
            <li>
              <div class="help-icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="none" viewBox="0 0 48 48">
              ...
              <div class="help-title">代码块</div>
              <div class="help-content"><code>```编程语言↵</code></div>
            </li>
            <li>
              <div class="help-icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="none" viewBox="0 0 48 48">
            ...
            <li>
              ...
              <div class="help-title">Mermaid图表</div>
              <div class="help-content"><code>```mermaid</code></div>
            </li>
          </ul>
        </div>
      </a-drawer>
      ...
      <MdEditor
        ...
        id="mdeditor"
        ref="editorRef"
        v-model="updatePostForm.content"
        class="md-editor"
        :toolbars="toolbars"
      ...
      >
        <template #defToolbars>
          ...
          <ThemeExtension @select="selectTheme" />
  ...
  <a-modal :visible="showResultModalVisible" :hide-cancel="true" :footer="false" :closable="false">
    ...
    <div class="flex flex-col justify-center items-center">
      ...
      <div class="flex flex-row justify-center items-center mt-[20px]">
        <a-button class="mx-[10px]" type="primary" @click="gotoUrl('PostDetail', { postId: updatePostForm.postId })"
          >查看帖子详情</a-button
        >
...
const plateList = ref<Array<plateDto>>([]);
...
const changeSubject = (subjectId: string | number | boolean) => {
  updatePostForm.value.plateId = undefined;
  const subjectIndex = subjectList.value.findIndex((subjectInfo: any) => subjectInfo.subjectId === subjectId);
  plateList.value = subjectList.value[subjectIndex].plates;
};
...
onMounted(async () => {
  ...
  getMyColumn();
  postApi.PostDetailByCreator({ postId }).then((res) => {
    if (res.code === 0) {
...
const changeEditorType = (type: number) => {
  if (type === 2) {
    ...
    setTimeout(() => {
      editorType.value = 1;
      updatePostForm.value.editorType = 'rich';
      setLoading(false);
    }, 1500);
  } else {
    editorType.value = 2;
    updatePostForm.value.editorType = 'markdown';
    setLoading(false);
  }
  updatePostForm.value.content = undefined;
};
  ...
  const res = await Promise.all(
    files.map((file) => {
      return new Promise((rev, rej) => {
        ...
        service
          .post('/default/file/upload/file', form, {
            headers: {
...
const uploadCoverSuccess = (fileItem: any) => {
  if (fileItem.response.data.url) {
    updatePostForm.value.cover = fileItem.response.data.url;
  }
};
...
const toolbarsExclude = ref<Array<any>>(['save', 'github']);

const selectTheme = (theme: string) => {
  previewTheme.value = theme;
  updatePostForm.value.markdownConfig.theme = theme;
};
</script>
