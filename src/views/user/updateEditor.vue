<template>
  <div v-loading="loading" class="editor">
    <div class="header flex flex-row justify-between items-center">
      <a-input v-model="postForm.title" class="title-input" placeholder="请输入文章标题" size="large"></a-input>
      <div class="pr-5">
        <!-- <a-popconfirm
          ...
          <template #content>
            <div class="demo-arrow w-100">
              <div class="header h-12 border-solid border-b-gray-200 border-b w-full p-0">
                <span class="leading-[3rem] block font-bold text-base text-center">文章更新</span>
              </div>
              <div class="w-11/12 mx-auto my-5">
                <a-form ref="postFormRef" :model="postForm" auto-label-width>
                  ...
                  <a-form-item field="columns" label="文章封面">
                    <a-upload
...
onMounted(async () => {
  ...
  console.log(postId, '编辑');
  postApi.getTagList({ ...tagPagination }).then((res) => {
    if (res.code === 0) {
  ...
  postApi.PostDetailByCreator({ postId }).then((res) => {
    if (res.code === 0) {
      const { title, history, subjectId, columns, plates, tagList, cover, desc, postId } = res.data;
      postForm.content = history[0].content;
      postForm.title = title;
      postForm.subjectId = subjectId;
      const subjectIndex = subjectList.value.findIndex((subjectInfo: any) => subjectInfo.subjectId === subjectId);
      plateList.value = subjectList.value[subjectIndex].plates;
      postForm.plateId = plates[0].plateId;
      postForm.columnId = columns[0].columnId;
      postForm.tags = tagList;
      postForm.cover = cover;
      postForm.desc = desc;
