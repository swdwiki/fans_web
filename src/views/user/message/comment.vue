    ...
    >
      <div class="child-top-nav">
        <router-link
          ...
          :key="index"
          :to="{ name: 'CommentMessage', query: { type: nav.value } }"
      ...
      <template v-if="selectNav === 'post'">
          ...
          <div v-for="item in comments" :key="item.commentId" v-loading="loading" class="">
            <div v-if="item.post" class="like-card">
              <div class="flex flex-row justify-start items-start">
                <a-avatar :size="40"> <img :src="item.commentAccount.avatar" /> </a-avatar>
                <div class="info flex flex-col justify-center items-start ml-[10px]">
                  <div class="name-line flex flex-row justify-start items-center">
                    <router-link :to="`/u/${item.commentAccount.userId}`" class="text-base font-bold">{{
                      item.commentAccount.nickname
                    }}</router-link>
                    <div class="flex flex-row items-center">
                      <span class="text-sm text-gray-600 ml-[10px]">评论了你的帖子</span>
                      <router-link
                        class="ml-[5px] text-gray-800 font-bold block leading-[30px]"
        ...
        <template v-else>
          <a-empty class="mt-[200px]" description="当前暂无尺牍评论" />
        </template>
      </template>
              ...
              <div class="flex flex-row justify-start items-start">
                ...
                <div class="info flex flex-col justify-center items-start ml-[10px]">
                  <div class="name-line flex flex-row justify-start items-center">
                    ...
                    <div class="flex flex-row items-center">
                      <span class="text-sm text-gray-600 ml-[10px]">评论了你的尺牍</span>
                      <router-link
                        class="ml-[5px] text-gray-800 font-bold block leading-[30px]"
                        :to="{ name: 'stickDetail', params: { id: item.stick.stickId } }"
                      >
                        <span v-html="renderedContent(item.stick.content)"></span>
                      </router-link>
                    </div>
                  </div>
                  <div>
                    <router-link
                      :to="{ name: 'stickDetail', params: { id: item.stick.stickId } }"
                      class="text-black text-base block leading-[30px]"
                      v-html="htmlToText(renderedContent(item.content))"
                    ></router-link>
                  </div>
                  <span class="mt-[5px] text-gray-500 block">{{ dayjs(item.createdAt).fromNow() }}</span>
                </div>
              ...
              <div class="flex flex-row justify-start items-start">
                ...
                <div class="info flex flex-col justify-center items-start ml-[10px]">
                  <div class="name-line flex flex-row justify-start items-center">
                    ...
                    <div class="flex flex-row items-center">
                      <span class="text-sm text-gray-600 ml-[10px]">评论了你的同人作品</span>
                      <router-link
                        class="ml-[5px] text-gray-800 font-bold block leading-[30px]"
...
const htmlToText = (html: string) => {
  ...
  // 移除图片和不需要的元素
  tempDiv
    .querySelectorAll('script, style, noscript, img, svg, iframe, video, audio')
...
// 发送已读消息
const readCommentRecord = (type: string) => {
  const recordIds = [];
  comments.value.forEach((comment: any) => {
    if (!comment.readStatus) {
      recordIds.push(comment.commentId);
    }
  });
  defaultApi
    .sendReadCommentRecord({
      type,
      recordIds,
    })
    .then((res) => {
...
const fetchData = async (params = { current: 1, pageSize: 10, type: 'post' }) => {
  setLoading(true);
  const { data } = await defaultApi.getCommentsList(params);
  pagination.current = Number(params.current);
  pagination.total = Number(data.total);
  comments.value = data.list;
  readCommentRecord(selectNav.value);
